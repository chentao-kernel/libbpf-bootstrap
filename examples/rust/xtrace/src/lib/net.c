...
typedef void (*handle_event_func_t)(void *ctx, int cpu, void *data, __u32 data_sz);
...
struct mproto_t net_mproto[MAX_PROTOCOL_NUM] = {
	{ 0, "dns" }, { 1, "http" }, { 2, "redis" }, { 3, "kafka" }, { 4, "mysql" },
};
...
static int user_config_proto(int config_fd)
{
	...
	for (protocol = ProtoHTTP; protocol < NumProto; protocol++) {
		...
		if (ret)
			net_log(LOG_TYPE_WARN, "Could not update map for protocol %d, err: %s",
...
static int user_config_info(int config_fd)
{
	...
	if (ret)
		net_log(LOG_TYPE_WARN, "Could not update map for config_info, err: %s",
			strerror(-ret));
	else
...
static int set_events_cont(int perf_fd, int page_count, handle_event_func_t event_func,
			   handle_lost_func_t lost_func, struct perf_buffer **pbuf)
{
	if (page_count <= 0) {
		page_count = 128;
	}
...
int32_t ebpf_start(void)
{
	...
	err = set_events_cont(bpf_map__fd(obj->maps.connect_ctrl_events_map),
			      env.page_count[CTRL_HAND], handle_ctrl_event, handle_lost_ctrl_event,
			      &(env.pbs[CTRL_HAND]));
	if (err)
		return err;
	err = set_events_cont(bpf_map__fd(obj->maps.connect_data_events_map),
			      env.page_count[INFO_HANDLE], handle_data_event,
			      handle_lost_data_event, &(env.pbs[INFO_HANDLE]));
	if (err)
		return err;
	err = set_events_cont(bpf_map__fd(obj->maps.connect_stats_events_map),
			      env.page_count[STAT_HAND], handle_stat_event, handle_lost_stat_event,
			      &(env.pbs[STAT_HAND]));
	if (err)
		return err;
...
void ebpf_config(int32_t opt1, int32_t opt2, int32_t params_count, void **params,
		 int32_t *params_len)
{
	struct net_bpf *obj = env.obj;
	int32_t *value;
...
static int cleanup_dog_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.cleanup_dog_probe = bpf_program__attach_uprobe(
		obj->progs.cleanup_dog_probe, false, 0, so, uprobe_offset); // 0 for self
...
static int update_conn_addr_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.update_conn_addr_probe = bpf_program__attach_uprobe(
		obj->progs.update_conn_addr_probe, false, 0, so, uprobe_offset); // 0 for self
...
static int disable_process_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.disable_process_probe = bpf_program__attach_uprobe(
		obj->progs.disable_process_probe, false, 0, so, uprobe_offset); // 0 for self
...
static int update_conn_role_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.update_conn_role_probe = bpf_program__attach_uprobe(
		obj->progs.update_conn_role_probe, false, 0, so, uprobe_offset); // 0 for self
