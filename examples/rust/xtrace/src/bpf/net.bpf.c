...
/* for rust compile export C struct */
struct conn_ctrl_event_t _event = {};
...
static __always_inline void *get_sock_from_fd(int fd_num)
{
	void *file = NULL;
...
{
	u32 index = 0;
	struct config_info_t *config = NULL;

	config = bpf_map_lookup_elem(&config_info_map, &index);
	if (config == NULL) {
		return TgidUndefine;
	}
	if (config->pid == tgid) {
...
static __always_inline uint64_t get_start_time()
{
	...
	bpf_probe_read(&group_leader_ptr, sizeof(struct task_struct *), (uint8_t *)task + gl_off);

	uint64_t st_off = offsetof(struct task_struct, start_time);
	uint64_t start_time = 0;
	bpf_probe_read(&start_time, sizeof(uint64_t), (uint8_t *)group_leader_ptr + st_off);
...
static __always_inline void init_conn_id(uint32_t tgid, int32_t fd, struct connect_id_t *conn_id)
{
	conn_id->tgid = tgid;
	conn_id->fd = fd;
...
static __always_inline enum support_type_e detect_mysql(const char *buf, size_t count,
							struct connect_info_t *conn_info)
{
...
static __always_inline enum support_type_e detect_kafka(const char *buf, size_t count,
							struct connect_info_t *conn_info)
{
	bool use_prev_buf = (conn_info->prev_count == 4) &&
...
static __always_inline bool is_redis(const char *buf, size_t count)
{
	...
	if (first_byte != '+' && first_byte != '-' && first_byte != ':' && first_byte != '$' &&
	    first_byte != '*') {
		return false;
	}
	char last_byte;
...
static __always_inline enum support_type_e detect_pgsql_start_mesg(const char *buf, size_t count)
{
	int min_msg_len = 12;
...
static __always_inline enum support_type_e detect_pgsql_query_mesg(const char *buf, size_t count)
{
	uint8_t tag_q = 'Q';
...
static __always_inline enum support_type_e detect_pgsql_reglr_mesg(const char *buf, size_t count)
{
...
static __always_inline enum support_type_e detect_pgsql(const char *buf, size_t count)
{
...
static __always_inline enum support_type_e detect_mongo(const char *buf, size_t count)
{
 ...
 * 1 byte: req:0 resp:1
 */
static __always_inline enum support_type_e detect_hsf(const char *buf, size_t count)
{
 ...
 * 1bit: req:1 resp:0
 */
static __always_inline enum support_type_e detect_dubbo(const char *buf, size_t count)
{
...
static __always_inline struct protocol_type_t detect_proto(const char *buf, size_t count,
							   struct connect_info_t *conn_info)
{
...
add_data_event(enum support_syscall_e src_fn, enum support_direction_e direction,
...
static __always_inline struct connect_info_t *build_conn_info(uint32_t tgid, int32_t fd)
{
	...
	struct connect_info_t *tmp =
		(struct connect_info_t *)bpf_map_lookup_elem(&connect_info_map, &tgid_fd);
	if (tmp)
		return tmp;
...
static __always_inline bool need_trace_protocol(const struct connect_info_t *conn_info)
{
	...
	struct config_info_t *info = NULL;
	bool ret = false;
	...
	info = bpf_map_lookup_elem(&config_info_map, &index);
	if (!info)
		return false;
	if (protocol < NumProto)
		ret = info->proto[protocol] > 0 ? true : false;
	return ret;
}
					 ...
					 struct tg_info_t *tg_role)
{
	...
	if (conn_info.is_sample) {
		bpf_perf_event_output(ctx, &connect_ctrl_events_map, BPF_F_CURRENT_CPU, &ctrl_event,
					      ...
					      ssize_t return_bytes)
{
	...
	bool real_threshold =
		(total_bytes >= conn_info->last_output_rd_bytes + conn_info->last_output_wr_bytes +
					ConnStatsBytesThreshold) ||
		(total_pkts >= conn_info->last_output_rd_pkts + conn_info->last_output_wr_pkts +
				       ConnStatsPacketsThreshold);
	if (real_threshold) {
		struct conn_stats_event_t *event = add_conn_stats(conn_info);
		if (event != NULL) {
			bpf_perf_event_output(ctx, &connect_stats_events_map, BPF_F_CURRENT_CPU,
					      event, sizeof(struct conn_stats_event_t));
		}
...
static __always_inline void add_close_event(struct trace_event_raw_sys_exit *ctx,
					    struct connect_info_t *conn_info)
{
	struct conn_ctrl_event_t ctrl_event = {};
...
static __always_inline void trace_exit_connect(struct trace_event_raw_sys_exit *ctx, uint64_t id,
...
static __always_inline void trace_reserve_conn(struct trace_event_raw_sys_exit *ctx, uint64_t id,
					       const struct conn_param_t *conn_param)
{
	uint32_t tgid = id >> 32;
...
static __always_inline void trace_exit_close(struct trace_event_raw_sys_exit *ctx, uint64_t id,
					     const struct close_param_t *close_param)
{
	...
	* 未等到read配对,超时提前close的情况此处需要清理map中记录的数据
	...
	if (need_trace_family(conn_info->addr.sa.sa_family) || conn_info->wr_bytes != 0 ||
	    conn_info->rd_bytes != 0) {
		add_close_event(ctx, conn_info);
		if (conn_info->last_output_rd_pkts + conn_info->last_output_wr_pkts !=
			...
			if (stats_event != NULL) {
				...
				bpf_perf_event_output(ctx, &connect_stats_events_map,
...
static __always_inline void trace_exit_accept(struct trace_event_raw_sys_exit *ctx, uint64_t id,
			      ...
			      struct conn_data_event_t *resp_data, size_t copied)
{
	...
	if (resp_data->attr.proto_rescode > 0 && resp_data->attr._type == TypeResponse) {
		bpf_perf_event_output(ctx, &connect_data_events_map, BPF_F_CURRENT_CPU, resp_data,
				      sizeof(resp_data->attr) + copied);
	}
	return 0;
}
...
static int read_write_match(void *ctx, struct conn_data_event_t *event, size_t copied)
{
	...
	/* read update */
	if (func == FuncWrite || func == FuncSecuritySendMsg || func == FuncSendMsg ||
	    func == FuncSend || func == FuncSendTo) {
		struct conn_data_event_t *data = bpf_map_lookup_elem(&trace_data_map, &tgid_fd);
		if (!data) {
	...
	if (ts > NSEC_PER_SEC && !check_inner(ip)) {
		bpf_perf_event_output(ctx, &connect_data_events_map, BPF_F_CURRENT_CPU, event,
...
static __always_inline void output_buf(struct trace_event_raw_sys_exit *ctx,
				       const enum support_direction_e direction, const char *buf,
				       const size_t buf_size, struct connect_info_t *conn_info,
				       struct conn_data_event_t *event)
{
	int bytes_sent = 0;
	unsigned int i;
...
static __always_inline void output_iovec(struct trace_event_raw_sys_exit *ctx,
					 ...
					 const size_t total_size, struct connect_info_t *conn_info,
					 struct conn_data_event_t *event)
{
	int bytes_sent = 0;
#pragma unroll
...
static __always_inline void trace_exit_data(struct trace_event_raw_sys_exit *ctx, uint64_t id,
					    const enum support_direction_e direction,
					    const struct data_param_t *data_param,
					    ssize_t return_bytes, bool vecs)
{
	uint32_t tgid = id >> 32;
	...
	if (!vecs) {
		...
		const size_t buf_size = iov_cpy.iov_len < return_bytes ? iov_cpy.iov_len :
									       return_bytes;
	...
	if (need_trace_protocol(conn_info) || matched == TgidMatch) {
		struct conn_data_event_t *event =
			add_data_event(data_param->syscall_func, direction, conn_info);
		if (event == NULL) {
			return;
		}
		...
		if (!vecs) {
			...
			output_iovec(ctx, direction, data_param->iov, data_param->iovlen,
...
static __always_inline void test_bpf_syscall(void *ctx, uint64_t id, int32_t fd,
					     const struct sockaddr *addr, int32_t ret,
					     uint32_t funcid)
{
...
int kprobe_security_socket_sendmsg(struct pt_regs *ctx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	struct data_param_t *write_param = bpf_map_lookup_elem(&write_param_map, &id);
	if (write_param != NULL) {
...
int kprobe_security_socket_recvmsg(struct pt_regs *crx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	struct data_param_t *read_param = bpf_map_lookup_elem(&read_param_map, &id);
	if (read_param != NULL) {
...
#pragma unroll
	for (i = 0; i < CONN_CLEANUP_NUMS; i++) {
		...
		conn_info = (struct connect_info_t *)bpf_map_lookup_elem(&connect_info_map,
										&tgid_fd);
