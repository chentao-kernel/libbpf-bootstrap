 ...
 * @update_conn_offset 更新conn_addr函数偏移
 * @disable_proc_offset disbale proc 函数偏移
 * @update_role_offset 更新rol函数偏移
 */
int32_t ebpf_init(char *btf, int32_t btf_size, char *so, int32_t so_size, long uprobe_offset,
		long conn_addr_ofst, long disable_process_ofst, long conn_role_addre);
/*
 * @return int32_t 0:success, others:failed
 */
...
// key: connect_id,addr: ip地址， port: 端口，drop：是否采集
void  ebpf_update_conn_addr(struct connect_id_t *conn_id, union sockaddr_t *dest_addr, uint16_t port, bool drop);
