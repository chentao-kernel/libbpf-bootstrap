...
static int update_conn_addr_init(char *so, int32_t so_size,
				 long uporobe_offset);
static int disable_process_init(char *so, int32_t so_size, long uporobe_offset);
static int update_conn_role_init(char *so, int32_t so_size,
				 long uporobe_offset);
...
int bpf_net_print_fn(enum libbpf_print_level level, const char *format,
		     va_list args)
{
...
static void net_log(enum libbpf_print_level level, const char *format, ...)
{
	va_list args;
...
static void handle_lost_test_events(void *ctx, int cpu, __u64 lost_cnt)
{
	...
	if (env.lost_callback.func == NULL) {
		...
		env.lost_callback.func(env.lost_callback.custom_data, TEST_HAND,
				       lost_cnt);
	}
}
#endif
...
static void handle_lost_ctrl_event(void *ctx, int cpu, __u64 lost_cnt)
{
	...
	if (env.lost_callback.func == NULL) {
		...
		env.lost_callback.func(env.lost_callback.custom_data, CTRL_HAND,
				       lost_cnt);
	}
}
...
static void handle_lost_data_event(void *ctx, int cpu, __u64 lost_cnt)
{
	...
	if (env.lost_callback.func == NULL) {
		...
		env.lost_callback.func(env.lost_callback.custom_data, DATA_HAND,
				       lost_cnt);
	}
}
...
static void handle_lost_stat_event(void *ctx, int cpu, __u64 lost_cnt)
{
	...
	if (env.lost_callback.func == NULL) {
		...
		env.lost_callback.func(env.lost_callback.custom_data, STAT_HAND,
				       lost_cnt);
	}
}
...
static int user_config_proto(int config_fd)
{
	...
	for (protocol = ProtoHTTP; protocol < NumProto; protocol++) {
		...
		ret = bpf_map_update_elem(config_fd, &protocol, &env.enable,
					  BPF_ANY);
		if (ret)
			net_log(LOG_TYPE_WARN,
				"Could not update map for protocol %d, err: %s",
				protocol, strerror(-ret));
		else
			net_log(LOG_TYPE_INFO,
				"success to update map for protocol: %d %d\n",
...
static int user_config_info(int config_fd)
{
	...
	if (ret)
		net_log(LOG_TYPE_WARN,
			"Could not update map for config_info, err: %s",
			strerror(-ret));
	else
		net_log(LOG_TYPE_INFO,
			"success to update map for config_info: %d\n",
			env.protocol);
...
static void get_btf_path(void)
{
	...
	if (access(env.btf_custom_path, F_OK) != 0) {
		if (access("/sys/kernel/btf/vmlinux", F_OK) == 0) {
			snprintf(env.btf_custom_path,
				 sizeof(env.btf_custom_path),
...
static int set_events_cont(int perf_fd, int page_count,
			   handle_event_func_t event_func,
			   handle_lost_func_t lost_func,
			   struct perf_buffer **pbuf)
{
	...
	*/
	...
	pb = perf_buffer__new(perf_fd, page_count, event_func, lost_func, NULL,
			      NULL);
...
int32_t ebpf_start(void)
{
	...
	int err;
#ifdef NET_TEST
	err = set_events_cont(bpf_map__fd(obj->maps.test_map),
			      env.page_count[TEST_HAND], handle_test_event,
			      handle_lost_test_event, &(env.pbs[TEST_HAND]));
	if (err)
		return err;
#endif
	err = set_events_cont(bpf_map__fd(obj->maps.connect_ctrl_events_map),
			      env.page_count[CTRL_HAND], handle_ctrl_event,
			      handle_lost_ctrl_event, &(env.pbs[CTRL_HAND]));
	if (err)
		return err;
	err = set_events_cont(bpf_map__fd(obj->maps.connect_data_events_map),
			      env.page_count[DATA_HAND], handle_data_event,
			      handle_lost_data_event, &(env.pbs[DATA_HAND]));
	if (err)
		return err;
	err = set_events_cont(bpf_map__fd(obj->maps.connect_stats_events_map),
			      env.page_count[STAT_HAND], handle_stat_event,
			      handle_lost_stat_event, &(env.pbs[STAT_HAND]));
	if (err)
		return err;
	...
	for (j = 0; j < MAX_HAND; j++) {
		if (g_poll_callback_count < max_events && !*stop_flag) {
			...
			if (rst < 0 && errno != EINTR) {
				net_log(LOG_TYPE_WARN,
					"Error polling perf buffer: %d, hand_type:%d\n",
					rst, j);
				return rst;
			}
		}
...
static int cleanup_dog_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.cleanup_dog_probe =
		bpf_program__attach_uprobe(obj->progs.cleanup_dog_probe, false,
...
static int update_conn_addr_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.update_conn_addr_probe = bpf_program__attach_uprobe(
		obj->progs.update_conn_addr_probe, false, 0, so,
		uprobe_offset); // 0 for self
...
static int disable_process_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.disable_process_probe = bpf_program__attach_uprobe(
		obj->progs.disable_process_probe, false, 0, so,
		uprobe_offset); // 0 for self
...
static int update_conn_role_init(char *so, int32_t so_size, long uprobe_offset)
{
	...
	obj->links.update_conn_role_probe = bpf_program__attach_uprobe(
		obj->progs.update_conn_role_probe, false, 0, so,
		uprobe_offset); // 0 for self
