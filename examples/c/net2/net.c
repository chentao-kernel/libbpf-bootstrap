...
#define UPROBE_FUN_INIT(so, offset, func_probe) \
do { \
	env.obj->links.func_probe = bpf_program__attach_uprobe(env.obj->progs.func_probe, false, \
			0, so, offset); 	\
	err = libbpf_get_error(env.obj->links.func_probe); 	\
	if (err != 0) { \
		net_log(0, "uprobe %s failed: %s\n", #func_probe, strerror(-err)); \
		goto cleanup; \
	} \
} while(0)
...
int32_t ebpf_init(char *btf, int32_t btf_size, char *so, int32_t so_size, long uprobe_offset, long conn_addr_ofst, long disable_process_ofst, long conn_role_ofst)
{
	...
	UPROBE_FUN_INIT(so, uprobe_offset, cleanup_dog_probe);
	UPROBE_FUN_INIT(so, conn_addr_ofst, update_conn_addr_probe);
	UPROBE_FUN_INIT(so, disable_process_ofst, disable_process_probe);
	UPROBE_FUN_INIT(so, conn_role_ofst, update_conn_role_probe);
...
void ebpf_update_conn_addr(struct connect_id_t *conn_id, union sockaddr_t *addr, u_int16_t port, bool drop)
{
...
void ebpf_update_conn_role(struct connect_id_t *conn_id, enum support_role_e role)
{
