...
/*
static __always_inline void set_addr_pair_by_sock(struct sock *sk,
...
static __always_inline uint64_t get_start_time()
{
	...
	bpf_probe_read(&start_time, sizeof(uint64_t),
...
static __always_inline void init_conn_id(uint32_t tgid, int32_t fd,
...
static __always_inline void init_conn_info(uint32_t tgid, int32_t fd,
...
static __always_inline int net_bpf_strncmp(const char *src, const char *targ,
					   size_t n)
{
	size_t i;
#pragma unroll
...
static __always_inline enum support_type_e detect_http(const char *buf,
						       size_t count)
{
	if (count < 16) {
		return TypeUnknown;
...
static __always_inline enum support_type_e
detect_mysql(const char *buf, size_t count, struct connect_info_t *conn_info)
{
	...
	uint32_t len = use_prev_buf ? *((uint32_t *)conn_info->prev_buf) :
					    *((uint32_t *)buf);
	len = len & 0x00ffffff;
	...
	if (com == ComConnect || com == ComQuery || com == ComStmtPrepare ||
	    com == ComStmtExecute || com == ComStmtClose) {
		return TypeRequest;
	}
	return TypeUnknown;
}
...
static __always_inline enum support_type_e
detect_kafka(const char *buf, size_t count, struct connect_info_t *conn_info)
{
	bool use_prev_buf = (conn_info->prev_count == 4) &&
	...
	if (use_prev_buf && result == TypeRequest &&
...
static __always_inline enum support_type_e detect_dns(const char *buf,
						      size_t count)
{
...
static __always_inline enum support_type_e
detect_pgsql_start_mesg(const char *buf, size_t count)
{
	int min_msg_len = 12;
...
static __always_inline enum support_type_e
detect_pgsql_query_mesg(const char *buf, size_t count)
{
	uint8_t tag_q = 'Q';
...
static __always_inline enum support_type_e
detect_pgsql_reglr_mesg(const char *buf, size_t count)
{
							...
							size_t count)
{
	...
	if (opcode == MGUpdate || opcode == MGInsert || opcode == MGReserved ||
	    opcode == MGQuery || opcode == MGGetMore || opcode == MGDelete ||
	    opcode == MGKillGurs || opcode == MGCompres || opcode == MGMsg) {
		if (respon == 0) {
			return TypeRequest;
		}
	}
...
static __always_inline struct protocol_type_t
detect_proto(const char *buf, size_t count, struct connect_info_t *conn_info)
{
	...
	} else if ((proto_type.type = detect_mongo(pbuf, count)) !=
		   TypeUnknown) {
		proto_type.protocol = ProtoMongo;
	} else if ((proto_type.type = detect_mysql(pbuf, count, conn_info)) !=
		   TypeUnknown) {
		proto_type.protocol = ProtoMySQL;
	} else if ((proto_type.type = detect_kafka(pbuf, count, conn_info)) !=
		   TypeUnknown) {
	...
	} else if ((proto_type.type = detect_pgsql(pbuf, count)) !=
		   TypeUnknown) {
		proto_type.protocol = ProtoPGSQL;
	} else if ((proto_type.type = detect_dubbo(pbuf, count)) !=
		   TypeUnknown) {
...
update_proto_type(struct connect_info_t *conn_info,
		  enum support_direction_e direction, const char *buf,
		  size_t count)
{
	if (conn_info == NULL) {
		return false;
	...
	if (conn_info->protocol == ProtoHTTP &&
	    proto_type.protocol == ProtoUnknown) {
		return true;
	}
...
#ifdef NET_TEST
	net_bpf_print(
		"kernel detect conn protocol: pid:%d fd: %d  protocol:%d: \n",
		conn_info->conn_id.tgid, conn_info->conn_id.fd,
		conn_info->protocol);
#endif
	if (conn_info->role == IsUnknown && proto_type.type != TypeUnknown) {
		bool isrole = (direction == DirEgress) ^
			      (proto_type.type == TypeResponse);
...
{
	return family == AF_UNKNOWN || family == AF_INET ||
	       family == AF_INET6 || family == AF_UNIX;
}

static __always_inline struct conn_data_event_t *
add_data_event(enum support_syscall_e src_fn,
	       enum support_direction_e direction,
	       const struct connect_info_t *conn_info)
{
	uint32_t index = 0;
	struct conn_data_event_t *event =
		bpf_map_lookup_elem(&connect_data_events_heap, &index);
	if (event == NULL) {
		return NULL;
	}
	...
	event->attr.pos = (direction == DirEgress) ? conn_info->wr_bytes :
							   conn_info->rd_bytes;
...
static __always_inline struct conn_stats_event_t *
add_conn_stats(struct connect_info_t *conn_info)
{
	uint32_t index = 0;
	struct conn_stats_event_t *event =
		bpf_map_lookup_elem(&connect_stats_events_heap, &index);
	if (event == NULL) {
		return NULL;
	}
...
filter_data_sample(struct config_info_t *config_info,
		   struct connect_info_t *conn_info)
{
...
static __always_inline struct connect_info_t *build_conn_info(uint32_t tgid,
							      int32_t fd)
{
	...
	struct connect_info_t *tmp =
		(struct connect_info_t *)bpf_map_lookup_elem(&connect_info_map,
							     &tgid_fd);
	if (tmp)
		return tmp;
	...
	err = bpf_map_update_elem(&connect_info_map, &tgid_fd, &new_conn_info,
				  BPF_NOEXIST);
	if (err && err != -EEXIST)
		return NULL;

	return (struct connect_info_t *)bpf_map_lookup_elem(&connect_info_map,
							    &tgid_fd);
}

static __always_inline bool
need_trace_protocol(const struct connect_info_t *conn_info)
{
	if (conn_info->protocol == ProtoUnknown) {
		return false;
	}
	...
	uint64_t traced = 0;
	uint64_t config;
	uint64_t *tmp = (uint64_t *)bpf_map_lookup_elem(&config_protocol_map,
							&protocol);
	if (tmp != NULL)
		bpf_probe_read(&config, sizeof(uint64_t), tmp);

	if (bpf_map_update_elem(&config_protocol_map, &protocol, &traced,
				BPF_NOEXIST) == 0) {
		tmp = (uint64_t *)bpf_map_lookup_elem(&config_protocol_map,
						      &protocol);
		bpf_probe_read(&config, sizeof(uint64_t), tmp);
	}
					 ...
					 struct tg_info_t *tg_role)
{
	...
	if (conn_info.is_sample) {
		bpf_perf_event_output(ctx, &connect_ctrl_events_map,
				      BPF_F_CURRENT_CPU, &ctrl_event,
				      sizeof(struct conn_ctrl_event_t));
	}
}

static __always_inline void
output_conn_stats(struct trace_event_raw_sys_exit_comp *ctx,
		  struct connect_info_t *conn_info,
		  enum support_direction_e direction, ssize_t return_bytes)
{
	switch (direction) {
	case DirEgress:
	...
	bool real_threshold =
		(total_bytes >= conn_info->last_output_rd_bytes +
					conn_info->last_output_wr_bytes +
					ConnStatsBytesThreshold) ||
		(total_pkts >= conn_info->last_output_rd_pkts +
...
static __always_inline void
add_close_event(struct trace_event_raw_sys_exit_comp *ctx,
		struct connect_info_t *conn_info)
{
	struct conn_ctrl_event_t ctrl_event = {};
...
static __always_inline void
trace_exit_connect(struct trace_event_raw_sys_exit_comp *ctx, uint64_t id,
...
static __always_inline void
trace_reserve_conn(struct trace_event_raw_sys_exit_comp *ctx, uint64_t id,
		   const struct conn_param_t *conn_param)
{
	uint32_t tgid = id >> 32;
	...
	struct connect_info_t *conn_info =
		bpf_map_lookup_elem(&connect_info_map, &tgid_fd);
	if (conn_info != NULL) {
		return;
	}
...
static __always_inline void
trace_exit_close(struct trace_event_raw_sys_exit_comp *ctx, uint64_t id,
		 const struct close_param_t *close_param)
{
   ...
   */
	if (need_trace_family(conn_info->addr.sa.sa_family) ||
		...
		if (conn_info->last_output_rd_pkts +
			    conn_info->last_output_wr_pkts !=
		    conn_info->rd_pkts + conn_info->wr_pkts) {
			struct conn_stats_event_t *stats_event =
				add_conn_stats(conn_info);
			if (stats_event != NULL) {
				stats_event->conn_events =
					stats_event->conn_events | StatusClose;
				bpf_perf_event_output(
					ctx, &connect_stats_events_map,
					BPF_F_CURRENT_CPU, stats_event,
					sizeof(struct conn_stats_event_t));
			}
		}
	}
...
static __always_inline void
trace_exit_accept(struct trace_event_raw_sys_exit_comp *ctx, uint64_t id,
		  const struct accept_param_t *accept_param)
{
	...
	add_one_conn(ctx, accept_param->addr, accept_param->accept_socket,
		     &tg_role);
}

static __always_inline void
perf_output_user(struct trace_event_raw_sys_exit_comp *ctx,
		 const enum support_direction_e direction, const char *buf,
		 size_t buf_size, struct connect_info_t *conn_info,
		 struct conn_data_event_t *event)
{
	...
	if (copied > 0) {
		...
		bpf_perf_event_output(ctx, &connect_data_events_map,
				      BPF_F_CURRENT_CPU, event,
				      sizeof(event->attr) + copied);
	}
}
...
#pragma unroll
	for (i = 0; i < WRAPPER_LEN; ++i) {
		...
		bool remain = bytes_remain > CONN_DATA_MAX_SIZE &&
			      (i != WRAPPER_LEN - 1);
		const size_t current_size = remain ? CONN_DATA_MAX_SIZE :
							   bytes_remain;
		perf_output_user(ctx, direction, buf + bytes_sent, current_size,
				 conn_info, event);
...
static __always_inline void
output_iovec(struct trace_event_raw_sys_exit_comp *ctx,
	     const enum support_direction_e direction, const struct iovec *iov,
	     const size_t iovlen, const size_t total_size,
	     struct connect_info_t *conn_info, struct conn_data_event_t *event)
{
	int bytes_sent = 0;

#pragma unroll
	for (int i = 0; i < VEC_LEN && i < iovlen && bytes_sent < total_size;
	     ++i) {
		...
		perf_output_user(ctx, direction, iov_cpy.iov_base, iov_size,
				 conn_info, event);
		bytes_sent += iov_size;
...
static __always_inline void
trace_exit_data(struct trace_event_raw_sys_exit_comp *ctx, uint64_t id,
		const enum support_direction_e direction,
		const struct data_param_t *data_param, ssize_t return_bytes,
		bool vecs)
{
	uint32_t tgid = id >> 32;
	...
	struct connect_info_t *conn_info =
		build_conn_info(tgid, data_param->fd);
	if (conn_info == NULL) {
		return;
	}
	if (!need_trace_family(conn_info->addr.sa.sa_family)) {
	...
	bool drop = false;
	if (!vecs) {
		drop = update_proto_type(conn_info, direction, data_param->buf,
					 return_bytes);
	} else {
		struct iovec iov_cpy;
		bpf_probe_read(&iov_cpy, sizeof(struct iovec),
			       &data_param->iov[0]);
		const size_t buf_size = iov_cpy.iov_len < return_bytes ?
						      iov_cpy.iov_len :
						      return_bytes;
		drop = update_proto_type(conn_info, direction, iov_cpy.iov_base,
					 buf_size);
	}
	if (drop) {
		return;
	...
	if (need_trace_protocol(conn_info) || matched == TgidMatch) {
		struct conn_data_event_t *event = add_data_event(
			data_param->syscall_func, direction, conn_info);
		if (event == NULL) {
			return;
		}

		if (!vecs) {
			output_buf(ctx, direction, data_param->buf,
				   return_bytes, conn_info, event);
		} else {
			output_iovec(ctx, direction, data_param->iov,
				     data_param->iovlen, return_bytes,
				     conn_info, event);
		}
	}
	return;
...
#ifdef NET_TEST
static __always_inline void test_bpf_syscall(void *ctx, uint64_t id, int32_t fd,
					     const struct sockaddr *addr,
					     int32_t ret, uint32_t funcid)
{
  ...
  bpf_perf_event_output(ctx, &test_map, BPF_F_CURRENT_CPU, &tmp_nd, sizeof(struct test_data));
	...
	bpf_get_current_comm(&tmp_nd.com, TASK_COMM_LEN);
	bpf_perf_event_output(ctx, &test_map, BPF_F_CURRENT_CPU, &tmp_nd,
...
int tp_sys_exit_connect(struct trace_event_raw_sys_exit_comp *ctx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	const struct conn_param_t *conn_param =
		bpf_map_lookup_elem(&conn_param_map, &id);
	if (conn_param != NULL) {
...
int tp_sys_exit_accept(struct trace_event_raw_sys_exit_comp *ctx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	struct accept_param_t *accept_param =
		bpf_map_lookup_elem(&accept_param_map, &id);
...
#ifdef NET_TEST
	test_bpf_syscall(ctx, id, ctx->args[0], (struct sockaddr *)ctx->args[1],
			 0, 5);
#endif
	return 0;
}
...
int tp_sys_exit_close(struct trace_event_raw_sys_exit_comp *ctx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	const struct close_param_t *close_param =
		bpf_map_lookup_elem(&close_param_map, &id);
	if (close_param != NULL) {
...
int BPF_KPROBE(kprobe_security_socket_sendmsg)
{
	...
	if (write_param != NULL) {
		...
		uint64_t pid = id >> 32;
		net_bpf_print(
			"kernel security_socket_sendmsg print: pid:%d fd: %d: \n",
			pid, write_param->fd);
#endif
	}
#ifdef NET_TEST
...
int tp_sys_exit_write(struct trace_event_raw_sys_exit_comp *ctx)
{
	...
	if (write_param != NULL && write_param->real_conn) {
		...
		net_bpf_print("kernel sys_exit_write print: pid:%d fd: %d: \n",
			      pid, write_param->fd);
#endif
		trace_exit_data(ctx, id, DirEgress, write_param, return_bytes,
				false);
	}
...
int tp_sys_exit_read(struct trace_event_raw_sys_exit_comp *ctx)
{
	...
	if (read_param != NULL && read_param->real_conn) {
		trace_exit_data(ctx, id, DirIngress, read_param, return_bytes,
				false);
	}
...
int tp_sys_enter_sendto(struct trace_event_raw_sys_enter_comp *ctx)
{
	...
	bpf_probe_read(&addr, sizeof(struct sockaddr),
		       (struct sockaddr *)ctx->args[4]);
...
#ifdef NET_TEST
	test_bpf_syscall(ctx, id, ctx->args[0],
			 (const struct sockaddr *)ctx->args[4], 0, 17);
#endif
	return 0;
}
...
#ifdef NET_TEST
	test_bpf_syscall(ctx, id, ctx->args[0],
			 (const struct sockaddr *)ctx->args[4], 0, 19);
#endif
	return 0;
}
...
int tp_sys_enter_sendmsg(struct trace_event_raw_sys_enter_comp *ctx)
{
	...
	bpf_probe_read(&msghdr, sizeof(struct user_msghdr_comp),
		       (struct user_msghdr_comp *)ctx->args[1]);
	...
	if (pmsghdr != NULL) {
		if (pmsg_name != NULL) {
			...
			bpf_map_update_elem(&conn_param_map, &id, &conn_param,
					    BPF_ANY);
		}
		...
		bpf_map_update_elem(&write_param_map, &id, &write_param,
				    BPF_ANY);
	}
...
int tp_sys_exit_sendmsg(struct trace_event_raw_sys_exit_comp *ctx)
{
	...
	if (write_param != NULL) {
		...
		uint64_t pid = id >> 32;
		net_bpf_print(
			"kernel sys_exit_sendmsg print: pid:%d fd: %d: \n", pid,
			write_param->fd);
#endif
	}
	bpf_map_delete_elem(&write_param_map, &id);
...
int tp_sys_enter_sendmmsg(struct trace_event_raw_sys_enter_comp *ctx)
{
	...
	bpf_probe_read(&msghdr, sizeof(struct mmsghdr_comp),
		       (struct mmsghdr_comp *)ctx->args[1]);
	vlen = ctx->args[2];
...
int tp_sys_exit_sendmmsg(struct trace_event_raw_sys_exit_comp *ctx)
{
	...
	if (write_param != NULL && msgs_num > 0) {
		...
		bpf_probe_read(&return_bytes, sizeof(unsigned int),
			       write_param->msg_len);
		trace_exit_data(ctx, id, DirEgress, write_param, return_bytes,
				true);
	}
...
int tp_sys_enter_recvmmsg(struct trace_event_raw_sys_enter_comp *ctx)
{
	uint64_t id = bpf_get_current_pid_tgid();
	struct mmsghdr_comp msghdr;
	bpf_probe_read(&msghdr, sizeof(struct mmsghdr),
		       (struct mmsghdr *)ctx->args[1]);
	struct mmsghdr_comp *pmsghdr = &msghdr;
	struct sockaddr msg_name;
	struct sockaddr *pmsg_name = &msg_name;
...
int cleanup_dog_probe(struct pt_regs *ctx)
{
	...
	struct connect_id_t *conn_ids =
		(struct connect_id_t *)PT_REGS_PARM1(ctx);
...
#pragma unroll
	for (i = 0; i < CONN_CLEANUP_NUMS; i++) {
		...
		conn_info = (struct connect_info_t *)bpf_map_lookup_elem(
			&connect_info_map, &tgid_fd);
		if (conn_info != NULL &&
		    conn_info->conn_id.tgid == conn_id.tgid) {
			bpf_map_delete_elem(&connect_info_map, &tgid_fd);
		}
	}
}
...
int disable_process_probe(struct pt_regs *ctx)
{
	...
	if (drop) {
		...
		ret = bpf_map_update_elem(&config_tgid_map, &pid, &flag,
					  BPF_ANY);
	}
#ifdef NET_TEST
...
{
	struct connect_id_t conn_id = {};
	struct connect_id_t *conn_id_pr =
		(struct connect_id_t *)PT_REGS_PARM1(ctx);
...
/* uprobe auto attach should use uprobe/exec_path:ebpf_update_conn_addr[+offset]
 * here we set offset self, so only use SEC("uprobe")
 */
...
int update_conn_addr_probe(struct pt_regs *ctx)
{
	...
	conn_info = (struct connect_info_t *)bpf_map_lookup_elem(
		&connect_info_map, &tgid_fd);
	if (conn_info == NULL) {
		bpf_probe_read(&dest_addr, sizeof(union sockaddr_t),
			       dest_addr_pr);
		new_conn.addr = dest_addr;
		new_conn.conn_id = conn_id;
		new_conn.is_sample = !drop;
		new_conn.role = IsUnknown;
		bpf_map_update_elem(&connect_info_map, &tgid_fd, &new_conn,
				    BPF_ANY);
		...
		bpf_probe_read(&conn_info->addr, sizeof(union sockaddr_t),
			       dest_addr_pr);
