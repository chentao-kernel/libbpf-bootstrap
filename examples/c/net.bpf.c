...
int update_role_probe(struct pt_regs* ctx)
{
	struct connect_info_t *conn_info;
	struct connect_id_t conn_id = {0};
	struct connect_id_t *conn_id_ptr = (struct connect_id_t *)PT_REGS_PARM1(ctx);
	enum support_role_e role = PT_REGS_PARM2(ctx);
	uint64_t tgid_fd;
	
	bpf_probe_read(&conn_id, sizeof(conn_id), conn_id_ptr);
	tgid_fd = combine_tgid_fd(conn_id.tgid, conn_id.fd);
	...
	if (!conn_info)
		return 0;
	// update addr
	if (conn_info->conn_id.tgid == conn_id.tgid) {
		conn_info->role = role;
	}
...
int update_conn_probe(struct pt_regs* ctx)
{
	struct connect_info_t *conn_info;
	union sockaddr_t addr = {0};
	struct connect_id_t conn_id = {0};
	struct connect_id_t *conn_id_ptr = (struct connect_id_t *)PT_REGS_PARM1(ctx);
	union sockaddr_t *addr_ptr = (union sockaddr_t *)PT_REGS_PARM2(ctx);
	...
	if (!conn_info)
		return 0;
	// update addr
	if (conn_info->conn_id.tgid == conn_id.tgid && conn_info->conn_id.fd == conn_id.fd) {
		conn_info->addr = addr;
